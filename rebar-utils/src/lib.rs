...
pub fn path_to_delim<P>(path: &P) -> Result<String, Report>
where
    ...
    P: AsRef<OsStr> + AsRef<std::path::Path> + Debug,
{
    ...
    match p.extension() {
        Some(ext) => Ok(ext
            .to_str()
            .ok_or_else(|| eyre!("Failed to convert extension to string."))?
            .to_string()),
        None => Err(eyre!("Failed to parse extension from file: {path:?}")),
    }
}
